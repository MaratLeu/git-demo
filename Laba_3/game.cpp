    ...
    , ui(new Ui::game)
{
    ...
    QMenu* menu = menubar -> addMenu("Выход из игры");
    QAction* exit = menu -> addAction("Выход из игры");
    connect(exit, &QAction::triggered, this, &game::zubovich_window);

    live_1 = menubar -> addMenu("❤️");
    hide_1 = live_1 -> addAction("");

    live_2 = menubar -> addMenu("❤️");
    hide_2 = live_2 -> addAction("");

    live_3 = menubar -> addMenu("❤️");
    hide_3 = live_3 -> addAction("");

    count = 0;

    timer = new QTimer(this);
    connect(timer, &QTimer::timeout, this, &game::on_pushButton_2_clicked);
    timer -> start(10);
}
...
void game::on_pushButton_clicked()
{
    ...
    if (count == 1) {
        ...
        ui -> pushButton->setIcon(QIcon(":/img/image/zubovich_2.png"));
    }
    if (count == 2) {
        live_2 -> menuAction() -> setVisible(false);
        ui -> pushButton->setIcon(QIcon(":/img/image/zubovich_3.png"));
    }
    if (count == 3) {
