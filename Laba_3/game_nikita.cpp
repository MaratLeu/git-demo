...
#include "ui_game_nikita.h"
#include <QMenuBar>
#include <QCursor>
#include <random>
#include <QPropertyAnimation>
using namespace std;
game_nikita::game_nikita(QWidget *parent)
    : QDialog(parent)
    , ui(new Ui::game_nikita)
{
    ui->setupUi(this);
    ...
    connect(exit, &QAction::triggered, this, &game_nikita::nikita_window);
    ...
    connect(timer, &QTimer::timeout, this, &game_nikita::on_pushButton_2_clicked);
    timer -> start(1);
}

void game_nikita::on_pushButton_2_clicked()
{
    const int angle = 360;
    mt19937_64 mersenne(static_cast <unsigned int> (time(0)));

    int diff_x = mersenne() % 100; //(this->width() - ui->pushButton->x())
    int diff_y = mersenne() % 100; //(this->height() - ui->pushButton->y())
...
void game_nikita::on_pushButton_clicked()
{
    count++;

    if (count == 1) {
        live_1 -> menuAction() -> setVisible(false);
        ui -> pushButton->setIcon(QIcon(":/img/image/nikita_2.png"));
    }
    if (count == 2) {
        live_2 -> menuAction() -> setVisible(false);
        ui -> pushButton->setIcon(QIcon(":/img/image/nikita.png"));
    }
    if (count == 3) {
